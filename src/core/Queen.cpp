...
Bitboard Queen::generateMoves(Bitboard occupied, int square) const {
    ...
    for (auto& dir : directions) {
        for (int i = 1; i < 8; ++i) {
            ...
            if (globalRenderer && globalRenderer->getDebugEnabled()) {
                std::cout << "[DEBUG] Queen at " << square << " can move to " << newSquare << std::endl;
            }
        }
    }
    if (globalRenderer && globalRenderer->getDebugEnabled()) {
        std::cout << "[DEBUG] Queen moves for " << square << ": " << std::hex << moves.getValue() << std::dec << std::endl;
    }
    return moves;
}
